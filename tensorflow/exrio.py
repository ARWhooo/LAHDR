...
def _header(width, height, float_32=False, need_compress=False):
    ...
    head['screenWindowWidth'] = float(width)
    if not float_32:
        ...
        head['channels']['G'] = Imath.Channel(type=Imath.PixelType(Imath.PixelType.FLOAT))
        head['channels']['B'] = Imath.Channel(type=Imath.PixelType(Imath.PixelType.FLOAT))
        head['channels']['R'] = Imath.Channel(type=Imath.PixelType(Imath.PixelType.FLOAT))
    if not need_compress:
        head['compression'] = Imath.Compression(Imath.Compression.NO_COMPRESSION)
    else:
        head['compression'] = Imath.Compression(Imath.Compression.ZIP_COMPRESSION)
    return head
